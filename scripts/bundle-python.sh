...
# Run once on dev machine before Xcode archive/build.
# Output: Resources/bundled-python/python/ (~400-640 MB after cleanup)
#
# Usage:
#   ./scripts/bundle-python.sh
#
# The bundled Python is self-contained: no system Python, no brew, no conda.
# VMLXProcessManager launches it at runtime via Bundle.main.resourcePath.
...
BUNDLE_DIR="$PROJECT_DIR/Resources/bundled-python"
ENGINE_DIR="$PROJECT_DIR/Resources/vmlx_engine"

echo "==> Bundling Python $PYTHON_VERSION for Osaurus (vmlx-engine backend)"
...
# Install dependencies (lean: no torch, no mcp, no gradio, no mlx-audio, no soundfile)
...
# Install vmlx-engine from local source copy (--no-deps: all deps already installed)
echo "==> Installing vmlx-engine from source..."
if [ -d "$ENGINE_DIR" ]; then
  "$PYTHON" -m pip install --no-deps "$ENGINE_DIR"
else
  echo "WARNING: $ENGINE_DIR not found. Run Phase B2 first (copy vmlx_engine source)."
  echo "  Skipping engine install — you can install it later with:"
  echo "  $PYTHON -m pip install --no-deps $ENGINE_DIR"
fi
...
# torch/torchvision: vmlx-engine uses MLX, not PyTorch. transformers gracefully
# degrades without torch via is_torch_available().
...
# soundfile: requires libsndfile.dylib which isn't bundled.
...
rm -rf "$SITE/setuptools" 2>/dev/null || true
rm -rf "$SITE/setuptools"*.dist-info 2>/dev/null || true

# Keep pip intact but remove its tests
...
try:
    ...
    # Fix mRoPE dimension mismatch for MoE models (mlx-vlm 0.3.12 bug)
...
# 6. mlx_lm/models/ssm.py: Mamba state fixes
...
# Verify the Python shared library exists
...
echo "==> Verifying pip is functional..."
"$PYTHON" -s -m pip --version > /dev/null 2>&1 || { echo "ERROR: pip is broken after cleanup!"; exit 1; }
echo "  pip OK"

# Reject editable installs (prevents shipping dev-machine paths)
...
if [ -n "$EDITABLE_PTH" ]; then
  ...
  echo "  Fix: re-run bundle-python.sh from scratch."
  exit 1
fi
...
echo "    Next: Build Osaurus in Xcode (the bundle is copied into the app)"
